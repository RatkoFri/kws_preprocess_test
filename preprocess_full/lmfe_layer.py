...
class LMFELayerFULL(tf.keras.layers.Layer):

    def __init__(self, cfg: LMFEConfig):
        super(LMFELayerFULL, self).__init__()
        self.cfg = cfg
    ...
    def np_call(self, inputs):
        ...
        log_mels = np.log(mels, dtype=np.float32)
        dct_res = tf.signal.dct(log_mels)
        return np.expand_dims(dct_res, axis=-1)
