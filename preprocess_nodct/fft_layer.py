...
class FFTLayerNODCT(tf.keras.layers.Layer):
    def __init__(self, cfg: FFTConfig):
        super(FFTLayerNODCT, self).__init__()
        self.cfg = cfg
    ...
    def np_call(self, inputs):
        results = []
        for x in inputs:
            res = np.fft.fft(x * self.window_fn, norm="backward", axis=-1)
            res = (res.real ** 2) + (res.imag ** 2) + 1e-7
